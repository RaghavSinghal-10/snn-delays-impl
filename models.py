...
class SNN_Delay_Conv(nn.Module):
    ...
    def __init__(self, beta, learn_beta, threshold, learn_threshold, time_steps, surr):
        ...
        # define the network
        self.dcls3_1d_1 = Dcls3_1d(in_channels=2, out_channels=12, kernel_count=1, stride=1, 
                                   padding=0, dense_kernel_size=self.max_delay, dense_kernel_size=(5,5))
        ...
        self.dcls3_1d_2 = Dcls3_1d(in_channels=12, out_channels=32, kernel_count=1, stride=1,
                                    padding=0, dense_kernel_size=self.max_delay, dense_kernel_size=(5,5))
        ...
        self.dcls1d_1 = Dcls1d(in_channels=32*5*5, out_channels=100, kernel_count=1, stride=1, padding=0, dilated_kernel_size=self.max_delay, groups=1, bias=True, padding_mode='zeros', version='gauss')
        ...
        self.dcls1d_2 = Dcls1d(in_channels=100, out_channels=10, kernel_count=1, stride=1, padding=0, dilated_kernel_size=self.max_delay, groups=1, bias=True, padding_mode='zeros', version='gauss')
    ...
    def forward(self, data):
        ...
        for step in range(x_1.size(4)):
            in_1 = self.dropout_1(self.ln_1(self.mp_1(x_1[:,:,:,:,step])))
        ...
        x_2 = self.dcls2_2(x_2)
        #print(x_2.size())

        spk_rec_2 = []

        for step in range(x_2.size(4)):
            in_2 = self.dropout_2(self.ln_2(self.mp_2(x_2[:,:,:,:,step])))
        ...
        #print(torch.stack(spk_rec_3).size())

        #output of the form [time_steps, batch_size, features]
        return torch.stack(spk_rec_3), torch.stack(mem_rec_3)
    ...
    def decrease_sig(self, epoch, num_epochs, time_steps):
        ...
        # take sig from last dcls layer  
        sig = self.dcls1_1.SIG[0,0,0,0].detach().cpu().item()
